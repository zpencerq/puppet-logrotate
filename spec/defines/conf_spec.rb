  ...
  shared_examples 'error raised' do |param, p_type|
    context "=> 'foo'" do
      ...
      it {
        ...
        }.to raise_error(Puppet::Error, /'#{param}' expects a(|n) #{p_type} value/)
      }
    end
  end
  ...
  shared_examples 'error match' do |param, p_type|
    context "=> 'foo'" do
      ...
      it {
        expect {
          is_expected.to contain_file('/etc/logrotate.conf')
        }.to raise_error(Puppet::PreformattedError, /'#{param}' expects a (match for|value of type) #{p_type}/)
      }
    end
  end
      ...
      context '=> test.example.com' do
        ...
        %w(mailfirst maillast).each do |value|
          context "mail_when => #{value}" do
            let(:params) { {:mail => 'test@example.com',
                            :mail_when => value} }
            it { is_expected.to contain_file('/etc/logrotate.conf').
